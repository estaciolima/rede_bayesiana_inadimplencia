...
########################
# Tratar cardialidade de variáveis categóricas via Risk Binning
library(dplyr)
library(rlang) 

create_risk_bins <- function(df, var_cat, var_target, n_bins = 3) {
  
  var_cat_sym <- sym(var_cat)
  var_target_sym <- sym(var_target)
  new_risk_col_name <- paste0("Risco_", var_cat, "_BIN")
  
  # 1. Target Encoding e Binning (Criação da Tabela de Risco)
  # -------------------------------------------------------------------
  
  df_risk_map <- df %>%
    group_by(!!var_cat_sym) %>%
    summarise(
      taxa_inad_temp = mean(!!var_target_sym, na.rm = TRUE),
      .groups = 'drop'
    ) %>% 
    mutate(!!new_risk_col_name := ntile(taxa_inad_temp, n_bins) %>% paste0("Risk", .))
  
  # 2. Salvando a Tabela de Mapeamento de Risco (Mantido)
  
  file_path <- paste0('data/aux/', var_cat, '_risk_map.csv')
  write.csv(df_risk_map, file_path, row.names = FALSE)
  
  message(paste("Tabela de risco salva em:", file_path))
  
  # 3. Aplicando o Mapeamento de Risco ao Data Frame Principal (Mantido)
  # -------------------------------------------------------------------
  dfT <- df %>%
    left_join(df_risk_map %>% select(!!var_cat_sym, !!sym(new_risk_col_name)),by = var_cat) %>% 
    select(-c(!!var_cat_sym))
